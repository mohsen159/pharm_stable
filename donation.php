...
<head>
    ...
    <script type="text/javascript">
        function give(id, amount) {
            ...
            if (r !== null) {
                if (r <= amount) {
                    ...
                    xmlhttp.open("GET", window.location.origin + "/operation/give.php?id=" + id + "&n=" + r);
                    xmlhttp.send();
                } else {
                    alert(r + " is not a correctly formatted amount"); // just for test do somthing
                }
            }
        ...
        function edit_don(id , name , dci , amount  , exp ) {
            document.forms["edit_don"]["id"].value = id;
            document.forms["edit_don"]["name"].value = name ; 
            document.forms["edit_don"]["amount"].value = amount ; 
            document.forms["edit_don"]["dci"].value = dci ; 
            document.forms["edit_don"]["exp"].value = exp ;
        }
    </script>
                ...
                <div class="row">
                    <table id="donate" style="width:100%" class="display" class="display table table-hover my-0">
                        ...
                        <tbody>
                            <?php
                            $temp = $_SESSION['id'];
                            $sql = "SELECT * FROM `dunation` WHERE amount!=0 and id_ph = '$temp' ";
                  ...
                  <button class='btn btn-success btn-sm edit btn-flat' onclick=\"give('" . $row['id'] . "', '" . $row['amount'] . "')\">Give</button> 
      <button class='btn btn-info btn-sm edit btn-flat'  data-bs-toggle='modal' data-bs-target='#edit' 
                  onclick=\"edit_don('" . $row['id'] . "','" . $row['name'] . "','" . $row['dci'] . "','" . $row['amount']. "','" . $row['expiration'] .  "')\">More</button> </td>  ";
                                    echo "</tr> ";
                                }
                    ...
                    <?php include "model/donation_model.php"; ?>
...
<script>
    ...
    function p_auto(element) {
        autocomplete(element, products);
    }
